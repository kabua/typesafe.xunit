...
/// <summary>
/// Extension methods for generating various display name outputs
/// </summary>
/// <remarks>
/// Taken directly from Xunit.Sdk.TypeUtility, and then exposed more of its functionality.
/// </remarks>
public static class DisplayNameExtensions
{
    ...
    /// <summary>
    /// Formulates the extended portion of the display name for a test method. For tests with no arguments, this will
    /// return just an empty; for tests with arguments, attempts to format the arguments.
    /// </summary>
    /// <param name="method">The test method</param>
    /// <param name="arguments">The test method arguments</param>
    /// <returns>The full display name for the test method</returns>
    public static string GetArgumentsWithNames(this IMethodInfo method, object?[]? arguments)
    {
        if (arguments == null)
            return "";
    ...
    /// <param name="arguments">The test method arguments</param>
    /// <returns>The full display name for the test method</returns>
    public static string GetArgumentsWithoutNames(this IMethodInfo method, object?[]? arguments)
    {
        if (arguments == null)
            return "";
        ...
        for (idx = 0; idx < arguments.Length; idx++)
            displayValues[idx] = ArgumentFormatter.Format(arguments[idx]);
    ...
    private static object? TryConvertObject(object? argumentValue, Type parameterType)
    {
        ...
        if (parameterType.IsInstanceOfType(argumentValue))
        ...
        Type parameterType)
    {
        ...
        {
            var runtimeMethods = conversionDeclaringType.GetRuntimeMethods().ToList();
    ...
    public static string GetParameterName(IParameterInfo[] parameters, int index)
    {
    ...
    public static string ParameterToDisplayValue(string parameterName, object? parameterValue)
        => $"{parameterName}: {ArgumentFormatter.Format(parameterValue)}";

    public static string ResolveGenericDisplay(ITypeInfo[]? genericTypes)
    {
    ...
    public static bool ResolveGenericParameter(this ITypeInfo genericType, ITypeInfo methodParameterType, Type? passedParameterType, ref Type? resultType)
    {
    ...
    public static Type GetArrayElementTypeOrThis(Type? type)
    ...
    public static ITypeInfo StripElementType(ITypeInfo type, ref bool isArray)
    {
    ...
    /// <returns>True if resolving was successful, else false.</returns>
    public static bool ResolveMismatchedGenericArguments(this ITypeInfo genericType, Type? passedParameterType, ITypeInfo[] methodGenericTypeArguments, ref Type? resultType)
    {
    ...
    private static readonly ITypeInfo s_objectTypeInfo = Reflector.Wrap(typeof(object));
}

public static class CollectionExtensions
{
    public static List<T> CastOrToList<T>(this IEnumerable<T> source) => source as List<T> ?? source.ToList();

    public static T[] CastOrToArray<T>(this IEnumerable<T> source) => source as T[] ?? source.ToArray();
}
